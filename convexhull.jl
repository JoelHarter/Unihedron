...
function convex_hull(pts::AbstractVector{<:SVector{3, T}}) where T
    n = length(pts)
    n < 4 && error("3D Convex hull requires at least 4 non-coplanar points.")

    mat = Matrix(reduce(hcat, pts)')
    ch = chull(mat)
    
    # Extract triangular faces from QHull simplices matrix
    faces = Vector{Int}[Vector{Int}(row) for row in eachrow(ch.simplices)]
    
    return Polyhedron(pts, faces)
end

# Convenience overloads for Tuples or splatted SVectors
convex_hull(pts::Tuple) = convex_hull(collect(pts))
convex_hull(pts::SVector...) = convex_hull(collect(pts))
